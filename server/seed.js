...
async function main() {
    ...
    try {
        ...
        const productsCollection = client.db("pizzeria-app").collection("products");
        const categoriesCollection = client.db("pizzeria-app").collection("categories");

        let categories = ['breakfast', 'lunch', 'dinner', 'drinks']
                        .map((category) => { return { name: category } });
